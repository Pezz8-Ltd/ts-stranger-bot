import { ActionRowBuilder, ChatInputCommandInteraction, DMChannel, EmbedBuilder, GuildMember, InternalDiscordGatewayAdapterCreator, PermissionsBitField, TextChannel, VoiceBasedChannel, VoiceChannel } from "discord.js";
...
import { UserMetadata } from "./UserMetadataService";
...
export class StrangerServer {
    ...
    missingVoicePermissions(voiceChannel: VoiceBasedChannel): boolean {
        // Before joining, assert that the bot has speak and connection permissions to the channel
        const strangerBotMember: GuildMember | undefined = voiceChannel.guild.members.cache.get(strangerBot.user?.id as string);
        const channelPermissions: Readonly<PermissionsBitField> | undefined = strangerBotMember?.permissionsIn(voiceChannel);
        return !strangerBotMember || !channelPermissions || !channelPermissions.has("Connect") || !channelPermissions.has("Speak")
    }
    ...
    nicknameCommand(nickname: string) {
        this.userMetadata.storeNickname(nickname);
     ...
     * @param interaction slash command interaction received
     */
    async searchCommand(callerUserId: string, textChannel: TextChannel, voiceChannel: VoiceBasedChannel, interaction: ChatInputCommandInteraction | null, dmChannel: DMChannel | null): Promise<void> {
        ...
        if(this.isMatched()) return this.skipCommand(callerUserId, textChannel, voiceChannel, interaction, dmChannel);

        // Before joining, assert that the bot has speak and connection permissions to the channel
        if(this.missingVoicePermissions(voiceChannel)) {
            interaction?.reply({ content: "Missing permissions to join the voice channel!", ephemeral: true });
            return;
        }

        // Save userId, text and voice channel previously checked - retrieve current or db metadata
        this.userMetadata = await UserMetadata.init(callerUserId, dmChannel);
     ...
     * @param interaction slash command interaction received
     */
    async skipCommand(callerUserId: string, textChannel: TextChannel, voiceChannel: VoiceBasedChannel, interaction: ChatInputCommandInteraction | null, dmChannel: DMChannel | null): Promise<void> {
        ...
        if(this.isDisconnected()) this.botVoiceConnection = joinVoiceChannel({ selfDeaf: false, channelId: voiceChannel.id, guildId: this.guildId, adapterCreator: textChannel.guild?.voiceAdapterCreator as InternalDiscordGatewayAdapterCreator });
        console.log(this.botVoiceConnection)
    ...
    private _abort(stop: boolean = false): void {
        ...
        if(this.matchedStranger) {
            this.userMetadata.sendDM( this.getSummary() );
        ...
        if(isMatched && !stop) {
            this.dynamicInteraction.updateContentAndUpdate( this.getAbortContent() );
            // Remove matched stranger from metadata
            this.matchedStranger = undefined;
        } else {
            this.dynamicInteraction.updateContentAndUpdate( this.getStopContent() );
            this.info("Stop triggered, deleting StrangerServer");
            delete strangerServersMap[this.guildId];
        }
